...
export class HistoryPage implements OnInit {
  ...
  public getDrives() : Drive[] {
    ...
    return drives.filter( (drive) => {
      ...
      return (!this.select_year || parseInt(elem[0]) == this.select_year) && (!this.select_month || parseInt(elem[1]) == this.select_month);
    });
  }
  ...
  public incrementFilterPeriod(delta:number) {
    const today: dayjs.Dayjs = dayjs();  // const today = dayjs();
